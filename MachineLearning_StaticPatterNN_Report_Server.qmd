...
kableExtra::kable(anova.cca(rda(dat_train$Jaccard, H1_data)))
kableExtra::kable(anova.cca(rda(dat_train$Jaccard, H1_data))) # sign. p = 0.001
kableExtra::kable(anova.cca(rda(dat_train$Jaccard, H2_data))) # sign. p = 0.001
kableExtra::kable(anova.cca(rda(dat_train$Jaccard, H3_data))) # sign. p = 0.001
kableExtra::kable(anova.cca(rda(dat_train$Jaccard, H4_data))) # sign. p = 0.001
...
```{r}
#| label: recursive-feature-selection
#| eval: FALSE
...
# Plotting
install_and_load(c("gridExtra", "ggplot2"))
...
for (i in 1:39) {
  plots[[i]] <- ggplot() +
    geom_line(data = pp_list_ranger[[i]], aes(x = .data[[predictors[i]]], y = yhat)) +
    geom_line(data = pp_list_xgb[[i]], aes(x = .data[[predictors[i]]], y = yhat), linetype = "dashed") +
    geom_line(data = pp_list_gbm[[i]], aes(x = .data[[predictors[i]]], y = yhat), linetype = "dotted") +
    labs(x = paste(predictors[i]), y = "Partial Dependence", title = "Partial Dependence Plots") +
    theme_bw() +
    ylim(0, 1)
}
