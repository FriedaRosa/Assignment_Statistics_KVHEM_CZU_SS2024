...
for(j in seq_along(1:4)){
  ...
  saved_profiles[[j]] <- replicate(50, list())
  save_imp[[j]] <- replicate(50, list())
...
saveRDS(saved_profiles, file = "../data/02_rfe_saved_profiles.rds")
save.image(file = "../data/RData/rfe.RData")
```
...
res_top_vars <- list()
for(i in seq_along(rfe_res)){

res_top_vars[[i]] <- data.frame(
    var = row.names(varImp(saved_profiles[[i]]$fit$importance, scale = T)),
    Imp = varImp(saved_profiles[[i]])$Overall,
    include = c(rep_len(1, as.numeric(saved_profiles[[i]]$bestSubset)), 
                rep_len(0,  
                length(row.names(varImp(saved_profiles[[i]], scale = T)))-as.numeric(saved_profiles[[i]]$bestSubset))
            ),
    model = i
        )
}
...
ggplot(data = res_top_vars_df %>% filter(model == 1)) +
  # Background rectangles
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 10.5), 
            fill = "lightgray", alpha = 0.9) +
  # Box plot
  geom_col(aes(y = reorder(var, Imp), x = Imp)) +
  # Horizontal line
