...
```{r}
library(dplyr); library(caret); library(gbm)

# Jaccard 1
rangerM <- readRDS("../data/rds/03_rangerModel_Jaccard1all.rds")
xgbM <- readRDS("../data/rds/03_xgbModel_all_Jaccard1TLCUSTOM.rds")
gbmM <- readRDS("../data/rds/03_gbmModel_Jaccard1all.rds")
...
summary.gbm(gbmM$finalModel, order = TRUE) %>% 
  mutate(across(where(is.numeric), round, 3)) %>% 
  left_join(all_importances) %>% 
  ggplot(aes(x = rel.inf, y = Overall, col = var))+
  geom_point()


# Split importance between ALL predictors (n = 68, factors dummy coded)
vimp <- caret::varImp(gbmM, type = 1, scale = TRUE)
all_importances <- vimp$importance %>% arrange(desc(Overall)) %>% round(3)
all_importances$var <- rownames(vimp$importance %>% arrange(desc(Overall)))
avg_importance <-  
