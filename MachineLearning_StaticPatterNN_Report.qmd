...
pckgs <- c("dplyr", "ggplot2", "reshape2", "caret", "ggcorrplot", "recipes", "cito", "plotly", "igraph", "ggraph", "caretEnsemble", "randomForest", "ranger", "gbm", "xgboost", "vegan")
...
# Plot the results as a Venn diagram
plot(varpart_model)
showvarparts(varpart_model)
summary(varpart_model)
```

#### Trying a VENN-Diagram:
...
# Perform variation partitioning
varpart_model <- varpart(dat_train$Jaccard, H1_data, H2_data, H3_data, H4_data, scale = TRUE)

# Extract the R-squared values
r_squared <- round(varpart_model$part$fract$R.square, 4)

round(varpart_model$part$fract,2) %>% 
    kableExtra::kable() %>% 
    write.csv("./data/VariationPart_results2.txt")
# Create a list of the R-squared values
r_squared_list <- list(
  "H1" = r_squared[1],
  "H2" = r_squared[2],
  "H3" = r_squared[3],
...
venn.plot <- venn.diagram(r_squared_list, filename = "VennHypotheses_StaticPattern.pdf")
...
# Create data frames for each set of variables
H1_data <- dat_train %>% select(any_of(H1_vars))
H2_data <- dat_train %>% select(any_of(H2_vars))
H3_data <- dat_train %>% select(any_of(H3_vars))
H4_data <- dat_train %>% select(any_of(H4_vars))
...
# Plot the results as a Venn diagram using eulerr
# Extract the explained variances
explained_variance <- varpart_model$part$indfract$Adj.R.square

# Create a named vector of explained variances
venn_data <- c(
  "H1" = explained_variance[1],
  "H2" = explained_variance[2],
  "H3" = explained_variance[3],
  "H4" = explained_variance[4],
  "H1&H2" = explained_variance[5],
  "H1&H3" = explained_variance[6],
  "H1&H4" = explained_variance[7],
  "H2&H3" = explained_variance[8],
  "H2&H4" = explained_variance[9],
  "H3&H4" = explained_variance[10],
  "H1&H2&H3" = explained_variance[11],
  "H1&H2&H4" = explained_variance[12],
  "H1&H3&H4" = explained_variance[13],
  "H2&H3&H4" = explained_variance[14],
  "H1&H2&H3&H4" = explained_variance[15]
) 
venn_data[venn_data < 0] <- 0

# Plot the Venn diagram using eulerr
euler_plot <- euler(venn_data)
plot(euler_plot, fills = TRUE, labels = TRUE)

# Optional: Save the plot
pdf("VennHypotheses_StaticPattern.pdf")
plot(euler_plot, fills = TRUE, labels = TRUE)
dev.off()
...
p_pred_rangerModel <- ggplot(aes(observed, prediction), data = test_performance_rf)+
    geom_point()+
...
# Plotting
ggplot() +
  geom_line(data = pp_list_ranger[[1]], aes(x = D_AOO_a, y = yhat)) +
  geom_line(data = pp_list_xgb[[1]], aes(x = D_AOO_a, y = yhat), linetype = "dashed") +
  geom_line(data = pp_list_gbm[[1]], aes(x = D_AOO_a, y = yhat), linetype = "dotted") 


  geom_line(data = pp_list_ranger[[2]], aes(x = rel_occ_Ncells, y = yhat), col = 
"red") +
  geom_line(data = pp_list_xgb[[2]], aes(x = rel_occ_Ncells, y = yhat), linetype = "dashed", col = "red") +
  geom_line(data = pp_list_gbm[[2]], aes(x = rel_occ_Ncells, y = yhat), linetype = "dotted")
...
p <- as.data.frame(predict(modelsList, newdata = dat_test)) %>% cbind(dat_test$Jaccard)
