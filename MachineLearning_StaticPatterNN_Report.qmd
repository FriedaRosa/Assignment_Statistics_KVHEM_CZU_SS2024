...
indices <- createDataPartition(dat_train$Jaccard, p = 0.8, 5) # 10 resamples
```
...
rfRFE <-  list(summary1 = defaultSummary,
               ...
               summary2 = defaultSummary,
               fit = function(x, y, first, last, ...){
                 library(ranger)
                 ranger(dependent.variable.name = y, data = data.frame(x, y), importance = 'impurity', ...)
                 },
               pred = function(object, x)  predict(object, data.frame(x))$predictions,
               rank = function(object, x, y) {
                 vimp <- importance(object)
                 vimp <- data.frame(Overall = vimp)
                 vimp$var <- rownames(vimp)
                 vimp <- vimp[order(vimp$Overall, decreasing = TRUE), , drop = FALSE]
                 vimp
                 },
...
set.seed(42)
subsets <- c(1:5, 10, 15, 20, 25, 30, 35, 40, 45, 60)
